...
print("DBSCAN - silhouette scores and formed cluster when eps is %d and minPts is %d" % (iEps, minPts))
# Initialize the clusterer with n_clusters value and a random generator
# seed of 10 for reproducibility.
start_time = time.time()
db = DBSCAN(eps=iEps, min_samples=minPts).fit(X)
end_time = time.time()
...
for i in range(n_clusters):
    ...
    plt.suptitle(("Silhouette analysis by DBSCAN with eps=36 and minPts=11"
                  " and n_clusters = %d" % n_clusters),
